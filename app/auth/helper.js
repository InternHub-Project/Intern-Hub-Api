...
const sendEmail = async function (req, user, routeLink, messageLink, messagHeader , code="") {
    const tokenconfirm = await jwtGenerator({ userId: user.userId, TO: "user" }, 1 , 'h'); 
...
const sendComoanyEmail = async function (req, company, routeLink, messageLink, messagHeader , code="") {
    const tokenconfirm = await jwtGenerator({ companyId: company.companyId, TO: "company" }, 1 , 'h'); 
    const link = `${req.protocol}://${req.headers.host}${CONFIG.BASEURL}/${routeLink}/${tokenconfirm}`;
    const message = `<a href='${link}'>follow me to ${messageLink}</a> <br></br> ${code?`RecoveryCode: ${code}`:""}<br></br>`;
    const info = SEND_EMAIL_BY_NODEMAILER(
        company.email,
